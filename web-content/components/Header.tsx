      ...
      <div className="flex justify-between items-center px-6 md:px-20 pb-6 md:pb-7 shadow-1xl">
        ...
        <div className={`${loggedIn ? 'md:flex hidden' : 'hidden'} justify-between gap-x-3 items-center `}>
          ...
          <div className=" cursor-pointer">
            {profilePic ?
              <Image src={avatar} alt="logo" className="rounded-full w-8 h-8 md:w-10 md:h-10 object-cover" onClick={() => setProfileModal(!profileModal)} />
              :
              <div className=" bg-cs-grey-800 w-[38px] h-[38px] rounded-full flex justify-center items-center text-cs-grey-50" onClick={() => setProfileModal(!profileModal)}>{getNameAbbreviation()}</div>
            }
            {
              profileModal && (
                <div className=" bg-white border border-solid border-cs-grey-50 rounded-[10px] overflow-hidden absolute hidden md:block z-50">
                  <div className=" text-cs-grey-dark flex gap-x-2 items-center hover:bg-cs-grey-55 p-2 font-normal" onClick={() => {
                  ...
                  <div className=" text-cs-grey-dark flex gap-x-2 items-center hover:bg-cs-grey-55 p-2 font-normal"
                    onClick={logOut}>
        ...
        <div className=" relative  md:hidden cursor-pointer">
          {
            // for bigger screens
            profilePic && (
              <Image src={avatar} alt="logo" className={` cursor-pointer rounded-full w-8 h-8 md:w-10 md:h-10 object-cover ${loggedIn ? "block md:hidden" : "hidden"}`} onClick={() => setProfileModal(!profileModal)} />
            )}
          {
            !profilePic && loggedIn && (
              <div className=" bg-cs-grey-800 w-[38px] h-[38px] rounded-full flex justify-center items-center text-cs-grey-50 md:hidden cursor-pointer" onClick={() => setProfileModal(!profileModal)
              }>{getNameAbbreviation()}</div>
            )}
          {
            profileModal && (
