...
export default function GuestNameInput() {
  ...
  const handleInput = (input: React.ChangeEvent<HTMLInputElement>) => {
    ...
    setAuthData((prevState) => {
      if (prevState["First name"].length > 2 && prevState["Last name"].length > 2) {
      ...
      <div className="basis-full col-start-1 row-start-1">
        ...
        <form className=" lg:max-w-[415px]">
          <div className={` ${loggedIn ? 'hidden' : 'block'}`}>
            ...
            <AuthInput label="First name" action={handleInput} errorMessage={errMessage["First name"]} inputName="First name" inputType="text" placeHolder="Henry" value={authData["First name"]} />
            <AuthInput label="Last name" action={handleInput} errorMessage={errMessage["Last name"]} inputName="Last name" inputType="text" placeHolder="Kalu" value={authData["Last name"]} />
          </div>
          <div className={` ${loggedIn ? 'block' : 'hidden'} flex gap-x-4 items-center mt-[48px]`}>
            ...
            <p className=" text-cs-grey-dark font-medium">Signed in as {first_name} {surname}</p>
          </div>
          {loggedIn ? <SubmitButton text="Join meeting" action={handleRoute} activate={true} /> : <SubmitButton text="Join meeting" action={handleRoute} activate={activateButton} />}
