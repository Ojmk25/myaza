...
export default function CreateEventModal({
  ...
  const handleAddAttendee = () => {
    if (email && !attendees.some((a) => a.email === email)) {
      setAttendees([...attendees, { id: Date.now().toString(), email, name: email.split("@")[0] }])
      setEmail("")
    }
  }
      ...
      <div className="fixed inset-0 bg-black bg-opacity-50 flex items-start md:items-center justify-center z-50 p-4">
        <div className="bg-white rounded-lg shadow-xl w-full max-w-md max-h-[90vh] overflow-auto">
          <div className="flex justify-between items-center p-4 border-b sticky top-0 bg-white">
            <h2 className="text-2xl font-semibold">{event ? "Edit meeting" : "Schedule meeting"}</h2>
          ...
          <form onSubmit={handleSubmit} className="p-4 space-y-4">
            <div>
              <label className="text-sm font-medium text-cs-grey-100">Meeting Name</label>
              <input
                type="text"
                value={title}
            ...
            <div>
              <label className="text-sm font-medium text-cs-grey-100">Meeting Date</label>
              <input
                type="date"
                value={date}
            ...
            <div className="flex gap-4">
              <div className="flex-1">
                <label className="text-sm font-medium text-cs-grey-100">Start Time</label>
                <input
                  type="time"
                  value={startTime}
              ...
              <div className="flex-1">
                <label className="text-sm font-medium text-cs-grey-100">End Time</label>
                <input
                  type="time"
                  value={endTime}
            ...
            <div>
              <label className="text-sm font-medium text-cs-grey-100">Add People</label>
              <div className="flex gap-6">
                <div className="w-full">
                  <input
                    ...
                    className="h-[48px] px-[16px] py-[13px] border-cs-grey-300 border rounded-[10px] w-full placeholder:text-[#A4ABB7] placeholder:text-sm outline-nonew-full p-2 focus:outline-none focus:ring-1 focus:ring-purple-600"
                  />
                  <span className="py-2 text-xs text-cs-grey-500 font-normal">Add their email and press ENTER</span>
                </div>
                <button
                  type="button"
            ...
            {attendees.length > 0 && (
              <div className="space-y-2">
                {attendees.map((attendee, index) => (
                  <div
                    key={attendee.email}
                    className={`flex items-center justify-between p-2 rounded ${getColorForUser(index).bg}`}
                  >
            ...
            <div className="flex justify-end gap-6 pt-12">
              ...
              >
                {loading ? <span>Loading...</span> : <span>{event ? "Update" : "Create"}</span>}
              </button>
            </div>
          </form>
        </div>
